...
/*var mainDT = $('#mainDT').DataTable({
	ajax:{
		url : SERVERURL + "/recruitment/getRecruitmentMsgList",
		type : "POST"
	},
	pageLength: 3,
	columns:[{
...
$('#searchButton').click(function(){
	...
	if (inputText !="") {
			...
			$.ajax({
				url : SERVERURL+ '/recruitment/selectRecruitmentByPage?searchText='+ inputText,
				type : 'get',
				dataType : 'json',
				success: function(msg){
					if (msg.isSuccess == true) {
						if (msg.data==null) {
							...
							if (msg.page.totalCount <= msg.page.pageSize) {
								pageSizeThis = msg.page.totalCount;
							}else{
								pageSizeThis= msg.page.pageSize;
							}
							for (i = 0; i < pageSizeThis; i++) {
...
if (getUrlParam('searchText')!=null) {
	var searchTextPre = getUrlParam('searchText');
	$('input[name="search"]').val(searchTextPre);
	$('#searchButton').click();
...
function loadPage(i){
	...
	if (i>=0&& i<=allpages) {
		...
		$.ajax({
			url : SERVERURL+ '/recruitment/selectRecruitmentByPage?pageNow='+ i + '&searchText=' + inputText,
			type : 'get',
			dataType : 'json',
			success:function(msg){
				//------------------------------------------------------------------------
